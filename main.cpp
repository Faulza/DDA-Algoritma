...
int main(int argc, char **argv)
{
    ...
    cout<<"\nInputkan nilai x akhir: ";cin>>x2;
    cout<<"Inputkan nilai y akhir : ";cin>>y2;
    if((x1 < 0) || (y1 < 0) || (x2 < 0) || (y2 < 0)){
        cout<<"Nilai yang diinput ada yang bernilai negatif\n";
        goto awal;
    }
    cout<<endl;
    ...
    ///Set posisi window berada di tengah
...
///Pembulatan Nilai X atau Y, dimana pembulatan di C akan selalu kebawah
...
void dda(void) {
	...
	///hitung steps
	if (abs(dx) > abs(dy)) {///step mengikuti nilai dx/dy yang absolut(positif) terbesar
		steps = abs(dx); ///step dibuat absolut(positif)
	}else
        steps = abs(dy);
